...
public class Stich {
    ...
    public int getValue() {
        ...
        for (int i = 1; i < cardsCounter; i++) {
            if (cards[i].color != firstColor || cards[i].value == Integer.MAX_VALUE || cards[i].value == -1)//cards of different color, wizzards and narren do not count
                continue;
            sum += cards[i].value;
        }
        return sum;
    ...
    public Player getWinningPlayer() {//todo do not ignore trumpf
        ...
        for (int i = 1; i < cardsCounter; i++) {
            if (cards[i].color != firstColor && cards[i].value != Integer.MAX_VALUE)//cards of different color that are not a wizzard wil loose automatically
                continue;
